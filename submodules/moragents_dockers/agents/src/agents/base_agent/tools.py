...
def get_cdp_client() -> Cdp:
    ...
    with _client_lock:
        if not _client:
            ...
            try:
                _client = Cdp.configure(api_key, api_secret)
...
async def send_gasless_usdc_transaction(toAddress: str, amount: str) -> Tuple[Dict[str, Any], str]:
...
async def send_eth_transaction(toAddress: str, amount: str) -> Tuple[Dict[str, Any], str]:
