        ...
        try:
            if not request.wallet_address:
                return AgentResponse.needs_info(
                    content="Please provide a wallet address to check rewards."
                )

            logger.info(
                f"Checking rewards for wallet address: {request.wallet_address}"
            )
