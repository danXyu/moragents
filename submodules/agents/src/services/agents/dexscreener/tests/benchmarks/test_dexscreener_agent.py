...
from services.agents.dexscreener.models import (BoostedTokenResponse,
...
async def test_search_dex_pairs_success(dex_agent, make_chat_request):
    request = make_chat_request(
        content="Search for ETH/USDC pairs", agent_name="dexscreener"
    )
    ...
    with patch("services.agents.dexscreener.tools.search_dex_pairs") as mock_search:
        ...
        response = await dex_agent._execute_tool(
            DexScreenerToolType.SEARCH_DEX_PAIRS.value, {"query": "ETH/USDC"}
        )
...
async def test_get_latest_token_profiles(dex_agent, make_chat_request):
    ...
    with patch(
        "services.agents.dexscreener.tools.get_latest_token_profiles"
    ) as mock_profiles:
        ...
        assert response.response_type.value == "success"
        assert (
            response.action_type == DexScreenerToolType.GET_LATEST_TOKEN_PROFILES.value
        )
        assert response.metadata == mock_response.model_dump()
...
async def test_get_boosted_tokens(dex_agent, make_chat_request):
    ...
    with patch(
        "services.agents.dexscreener.tools.get_latest_boosted_tokens"
    ) as mock_boosted:
        ...
        assert response.response_type.value == "success"
        assert (
            response.action_type == DexScreenerToolType.GET_LATEST_BOOSTED_TOKENS.value
        )
        assert response.metadata == mock_response.model_dump()
...
async def test_error_handling(dex_agent, make_chat_request):
    with patch("services.agents.dexscreener.tools.search_dex_pairs") as mock_search:
        ...
        response = await dex_agent._execute_tool(
            DexScreenerToolType.SEARCH_DEX_PAIRS.value, {"query": "invalid"}
        )
