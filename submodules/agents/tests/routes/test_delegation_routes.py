...
async def test_chat_timeout(chat_request):
    with patch("src.routes.chat_routes.Delegator"), patch(
        "src.routes.chat_routes.ChatController"
    ) as mock_controller_class, patch("src.routes.chat_routes.logger") as mock_logger:
        mock_controller = mock_controller_class.return_value
